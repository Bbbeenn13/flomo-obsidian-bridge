...
日常生成阶段不会修改 Vault 的 `Daily_Note/`、`Thinking_Lab/`、`Wiki/` 或系统目录，也不会向 flomo 写入内容。只有用户明确批准后，审批脚本才会创建或追加当天 `YYYY.MM.DD_Codex.md`，并用日期标记阻止重复写入。`_Codex` 明确区分 AI 旁观整理与用户手写日记。

日记保留“今天的我、值得留下的矿、还没想完的地方、带到明天、可选卡点、素材索引”这组松弛结构，并附不超过 5 个关键词。它把 flomo 当作可能延迟、并不完全精确的人生素材，不要求用户按模板记录，也不强制量化或布置行动。
